# 논문 타당성 게이트 세트

Nature 논문 1편의 논리구조를 적대적으로 감사하고, 그 감사를 자기 설계에 되돌려
적용하는 과정에서 나온 **재사용 가능한 게이트 집합**이다. 어떤 논문에도 붙는다.
`design_v*.md`는 방법을 시험한 시험체이며 산출물이 아니다.

## 디렉토리 구조
```
research/paper-validity/
├─ README.md · HANDOFF.md · PROTOCOL.md      ← 여기서 시작
├─ policy/     REVISION_POLICY(R1–R33, REG-001–014) · R0 루브릭 · DEFENSE_STRATEGY · ERROR_LEDGER(30건)
├─ findings/   S4 원논문 감사 · R11–R14 공격 결과 · R15 의무 추적표 · 위원회 브리프 · 초기 수정 대장
├─ gates/      build_manuscript · manuscript_gate · cite_gate · contrast_expand
├─ design_v1…v5.3.md · design_lint.py · rule_check.py · obligation_diff.py · paper_lint.py · validate_gates.py
├─ fixtures/   R23 기지정상·기지결함 픽스처 (l13_vacuous_row.md는 현재 L13이 놓치는 결함)
└─ history/    COMMITS.md · (트랜스크립트·에이전트 기록은 비공개 저장소 결정 전까지 보류)
```
코드·설계·픽스처는 최상위에 그대로 둔다 — 린터·CI가 상대경로로 참조한다.

## 빠른 실행

```bash
python3 validate_gates.py            # R23 교정 하네스 — 전체 게이트 자기검증 (15개 기대값)
python3 gates/build_manuscript.py <템플릿.md> <registry.json> [out.md]
python3 gates/manuscript_gate.py <원고.md>            # 규율 하 원고 (strict)
python3 gates/manuscript_gate.py --audit <원고.md>    # 규율 밖 원고 (이행률만)
python3 gates/cite_gate.py <citations.json> <출처루트>
python3 rule_check.py                # 판정규칙 배타성
python3 paper_lint.py <원고.md>       # 배수/범주수/불확실성 단위
python3 design_lint.py design_v5.md --baseline=v3   # 설계문서 린트 L1–L12 (R27 기준선 명시)
python3 obligation_diff.py design_v4.md design_v5.md  # R15 의무 추적 후보
```

CI가 매 푸시마다 R23 조건을 재확인한다 → `.github/workflows/paper-validity-gates.yml`

## 게이트 (전부 기지답 코퍼스로 교정됨)

| 게이트 | 규칙 | 막는 것 | 검증 |
|---|---|---|---|
| `gates/build_manuscript.py` | R18·R19·R26 | 손으로 쓴 수치, 손서술 파생량 | 네이처 F1 결함(주장 2.8× vs 도출 1.90×)이 **구조적으로 발생 불가** |
| `gates/manuscript_gate.py` | R18·R19·R21·R24 | 맨 숫자, 파생량 서술, 태그 없는 경계 주장 | clean 0건 / defect 10건 전부 |
| `gates/cite_gate.py` | R20 | **인용 할루시네이션** — 개작·조작·대조불가·과단축 | 참 2건 통과 / 거짓 4계열 전부 검출 |
| `rule_check.py` | R16 | 판정규칙의 분기 중첩 | CI 격자 45,451개, v4 §1 중첩 1,225개(2.7%) 검출 |
| `paper_lint.py` | — | 배수 산술, 범주 수, 불확실성 단위 | 238,480자(논문+부록)에서 확정결함 재현 — **그 코퍼스는 저장소에 없다**(재배포 회피). 재현하려면 자신의 사본에 `python3 paper_lint.py <paper.md+supplement.md>` |
| `design_lint.py` | L1–L15 | 설계문서 회귀·고아 셀·기술용어 소실. **L13(R9)은 미검증** — `fixtures/l13_vacuous_row.md`를 통과시킴 | validate_gates가 이 미충족을 그대로 보고 |

## 먼저 읽을 것

| 파일 | 내용 |
|---|---|
| `PROTOCOL.md` | **단계별 하네스** — S0–S4 감사, B0–B6 반론 설계, C1–C3 자기 감사. 다음 논문에 그대로 적용 |
| `policy/ERROR_LEDGER.md` | **저자 자신의 거짓·과대 진술 16건 전수 대장**과 기계검출 가능성 |
| `policy/DEFENSE_STRATEGY.md` | 3층 방어(기계·선언·자원)와 공격 유형의 층별 배정 실측 |
| `policy/REVISION_POLICY.md` | 규칙 R1–R26, 회귀 대장 REG-001–008 (수정이 원래보다 나빠진 사례) |
| `findings/S4_final_report.md` | 대상 논문 감사 결과 (확정 FATAL 3건, 축자 인용 포함) |

## 실측된 것 (추정이 아님)

- 논리구조 공격의 **58%가 기계층** — 두 독립 측정(n=19, n=12)에서 동일
- 저자 자신의 오류 **16건 중 15건이 기계 게이트 사정권**
- **선언층 적중률 8%** — 사전 등록만으로는 닫히지 않는다 (추정 ~100%는 반증됨)
- 재작성 반복은 점수를 올리지 않았다. 실측 궤적(FATAL/MAJOR): v1 1/18 · v2 0/11 · v3 0/11 · v4 1/11 · v5.0 2/12 · v5.1 1/12 · v5.2 2/14 · **v5.3 1/14** (`findings/R14_findings.md`). 최저 라운드 채택 → **v5 계열 확정 ≤ 80**
- 처방을 쓰거나 따르는 과정에서 난 오류가 25건 중 12건(48%). 연속 두 라운드가 반대 처방을 낸 조항은 동결한다(R31)
- 게이트가 거짓말한 횟수 4회(E11·E17·E23·E30). L13은 3회 구현 모두 항진명제 → **R9는 산문 린트로 기계화 불가**(R32). L13은 릴리스 게이트에서 제외, 미검증 표기
- 게이트 구축 구간에서 오류가 가장 많이 났다 (16건 중 6건 = 38%).
  그중 2건은 게이트/하네스 자신이 잡았고, 4건은 사람 눈이 잡았다.

## 저장소만으로 재현 가능한 것 / 아닌 것
- 재현 가능: `validate_gates.py`(깨끗한 클론에서 확인, 현재 **미채택** — L13 공허행), CI 3단계, rule_check, cite_gate 픽스처, build_manuscript.
- 재현 불가: `paper_lint.py`의 네이처 결함 검출(원문 없음), 공격 라운드 R10–R14(에이전트 실행 기록은 `R1x_findings.md` 요약만).
- 에이전트 발견 중 저자가 직접 재현한 것: L13 항진(E23·E26), L15 no-op(E27), `clusters.min` 이중 정의, 실행 격리↔HTTP 코드 모순, §1 라벨 오류, 추정량 전개. **나머지는 에이전트 보고를 인용으로 대조했을 뿐 독립 재현하지 않았다.**

## 핵심 한계 (숨기지 않음)

1. **오류 대장의 분모는 관측 불가**하다. 발견되지 않은 거짓의 수는 알 수 없다.
   따라서 "15/16"은 검출률이 아니라 *검출된 것 중* 기계화 가능 비율이다.
   → R22(자기 최종검사 금지)만이 구조적 대응이며 **사람이 필요하다.**
2. **픽스처는 저자가 썼다.** 게이트는 저자가 생각해낸 결함 유형만 잡는다.
   미지의 결함 유형을 잡는다는 보장은 없다.
3. 연구 질문의 가치나 구성개념 조작화에 대한 이견은 어느 층도 막지 못한다.
   그것은 결함이 아니라 과학이다.

## 인용된 제3자 자료

`fixtures/source_excerpt.md`는 게이트 검증용 **단문 발췌**(701바이트)이며 전문을
재배포하지 않는다. 출처: Miao et al., Nature (2026), doi:10.1038/s41586-026-11044-y.
전문 대조가 필요하면 자신의 합법적 사본을 출처 루트로 지정해 사용한다.
